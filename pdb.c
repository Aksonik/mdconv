...
int PDBEntry::read(FILE *fptr, SelEnum selMode) {
  ...
  if (strlen(line)<50 || strncmp(line,"ATOM",4))
    return 0;
...
void PDBEntry::write(FILE *fptr) {
  if (strlen(atmname)>3) {
    fprintf(fptr,
     "ATOM %6d %-4s%c%-4s %5d   %8.3f%8.3f%8.3f%6.2f%6.2f      %4s\n",
     atminx,atmname,(alt==0)?' ':alt,resname,resnum,coor.x(),coor.y(),coor.z(),
     aux1,aux2,segid);
  } else {
    fprintf(fptr,
     "ATOM %6d  %-3s%c%-4s %5d   %8.3f%8.3f%8.3f%6.2f%6.2f      %4s\n",
     atminx,atmname,(alt==0)?' ':alt,resname,resnum,coor.x(),coor.y(),coor.z(),
     aux1,aux2,segid);
  }
}
